...
| 문제 유형                     | 원인 및 해결 방법                    |
| ----------------------------- | ------------------------------------ |
| 이슈가 칸반에서 이동되지 않음 | 브라우저 새로고침, 권한 확인         |
| 카테고리가 표시되지 않음      | 캐시 문제, 페이지 새로고침           |
| 외부 티켓 링크가 작동 안함    | 이슈 트래커 설정 확인, URL 형식 검토 |
| 필터가 적용되지 않음          | 필터 조건 재확인, And/Or 조건 점검   |
| 이슈 상세가 로드되지 않음     | 네트워크 상태 확인, 권한 문제 검토   |
...
| 권한 수준  | 가능한 작업                   |
| ---------- | ----------------------------- |
| **Admin**  | 모든 이슈 CRUD, 카테고리 관리 |
| **Editor** | 이슈 생성/수정, 상태 변경     |
| **Viewer** | 이슈 조회만 가능              |

#### 권한 문제 해결

1. **관리자 문의**: 권한이 부족한 경우 프로젝트 관리자에게 역할 변경 요청
2. **로그인 확인**: 세션 만료 시 재로그인 필요
3. **브라우저 캐시**: 권한 변경 후 브라우저 새로고침
...
- [대시보드 분석](./06-dashboard-analytics.md) - 이슈 통계 및 트렌드 분석
