...
| 문제 유형                 | 원인 및 해결 방법                               |
| ------------------------- | ----------------------------------------------- |
| 멤버 초대가 안됨          | 대상 사용자가 시스템에 등록되지 않음            |
| API 키가 작동하지 않음    | 키가 Inactive 상태이거나 잘못된 프로젝트에 사용 |
| 권한 변경이 반영되지 않음 | 브라우저 새로고침 또는 재로그인 필요            |
| 프로젝트 삭제가 막힘      | Admin 권한 없음 또는 이름 입력 오류             |
