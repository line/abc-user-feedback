...
:::info 권한 안내
웹훅 설정은 **프로젝트별**로 관리되며, 적절한 권한을 가진 사용자만 접근할 수 있습니다.
:::
...
| **Name**          | 웹훅 이름 (예: "User Feedback sync")                           |
| **URL**           | 알림을 받을 외부 엔드포인트                                    |
| **Event Trigger** | 구독 중인 이벤트 태그들 (Issue Added, Issue Status Changed 등) |
| **Created**       | 웹훅 생성일시                                                  |
...
| 항목     | 설명                             | 예시                                   |
| -------- | -------------------------------- | -------------------------------------- |
| **Name** | 웹훅 식별을 위한 이름            | `Slack 알림`, `개발팀 Discord`         |
| **URL**  | HTTP POST 요청을 받을 엔드포인트 | `https://hooks.slack.com/services/...` |
...
- 한 웹훅에서 여러 채널과 이벤트를 동시에 구독할 수 있습니다
- 필요에 따라 이벤트별로 별도 웹훅을 만들 수 있습니다
- 모든 채널의 동일 이벤트를 선택하려면 각 채널별로 체크해야 합니다
  :::
...
:::warning 주의사항
상태 변경은 즉시 적용되므로, 중요한 알림이 중단되지 않도록 주의하세요.
:::
...
:::danger 주의
삭제된 웹훅은 복구할 수 없으며, 해당 웹훅으로 전송되던 모든 알림이 중단됩니다.
:::
...
| 문제 상황            | 원인             | 해결 방법                           |
| -------------------- | ---------------- | ----------------------------------- |
| 알림이 오지 않음     | 웹훅이 Off 상태  | On/Off 토글을 확인하여 활성화       |
| HTTP 오류 발생       | 잘못된 URL       | URL 형식 및 접근 가능성 확인        |
| 일부 이벤트만 수신됨 | 이벤트 선택 누락 | 필요한 채널과 이벤트 모두 체크 확인 |
| 토큰 인증 실패       | 토큰 불일치      | Generate 버튼으로 새 토큰 생성      |

### 디버깅 체크리스트

- [ ] 웹훅이 활성화(On) 상태인지 확인
- [ ] Target URL이 HTTPS로 시작하는지 확인
- [ ] 필요한 이벤트가 모두 선택되었는지 확인
- [ ] 외부 서비스에서 웹훅 URL이 정상 작동하는지 테스트
- [ ] 네트워크 방화벽이나 접근 제한이 없는지 확인
