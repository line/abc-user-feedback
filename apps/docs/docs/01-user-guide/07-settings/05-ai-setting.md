...
:::info 사전 요구사항
AI 기능을 사용하려면 OpenAI 또는 Google Gemini의 유료 API 계정이 필요합니다.
:::
...
| 문제 유형            | 증상                  | 해결 방법                                   |
| -------------------- | --------------------- | ------------------------------------------- |
| **API 키 오류**      | 401 Unauthorized      | 새 키 발급 후 재설정                        |
| **연결 실패**        | Connection timeout    | Base URL을 기본값으로 재설정                |
| **응답 없음**        | 429 Too Many Requests | AI 제공자 콘솔에서 사용량 한도 확인         |
| **한국어 응답 실패** | 영어로만 응답         | System Prompt에 언어 지시사항 추가          |
| **비용 급증**        | 예상보다 높은 요금    | 사용 패턴 분석 후 불필요한 AI 필드 비활성화 |

### 연결 테스트

설정 완료 후 **Test Connection** 기능으로 정상 작동 여부를 확인하세요.
