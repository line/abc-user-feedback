...
| 문제 유형               | 원인                    | 해결 방법                               |
| ----------------------- | ----------------------- | --------------------------------------- |
| 연결 테스트 실패        | Base URL 오타           | URL 형식 재확인 (프로토콜, 포트 포함)   |
| 링크 클릭 시 404 오류   | Project Key 잘못됨      | Jira에서 정확한 Project Key 확인        |
| 링크 접근 권한 없음     | Jira 계정 권한 부족     | Jira 관리자에게 프로젝트 접근 권한 요청 |
| 티켓 번호 저장되지 않음 | 이슈 트래커 설정 미완료 | 이슈 트래커 관리에서 설정 완료          |
...
1. **Base URL 테스트**:
   ...
   ```
   브라우저에서 Base URL 직접 접속
   → Jira 로그인 화면이나 대시보드 표시되어야 함
   ```

2. **Project Key 확인**:

   ```
   Jira 프로젝트 페이지에서 URL 확인
   → .../projects/PROJECT_KEY/... 형태로 표시
   ```

3. **완전한 링크 테스트**:
   ```
   {Base URL}/browse/{Project Key}-1
   → 실제 이슈 페이지로 이동되는지 확인
   ```
