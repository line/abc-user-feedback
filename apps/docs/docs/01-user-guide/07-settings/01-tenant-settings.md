...
2. **정기적 권한 검토**: 분기별로 사용자 권한 및 접근 현황 점검
3. **SUPER 계정 최소화**: Super Admin은 2-3명 이하로 제한
4. **퇴사자 계정 즉시 삭제**: 퇴직 즉시 계정 삭제 또는 비활성화

#### 로그인 보안

1. **다중 인증 방식**: OAuth와 이메일 로그인 병행 사용 권장
2. **강력한 비밀번호 정책**: 복잡성 요구사항 유지
3. **세션 관리**: 공용 컴퓨터 사용 시 반드시 로그아웃
4. **정기적 설정 검토**: OAuth 설정 및 키 값 정기 갱신

### 운영 팁

#### 사용자 온보딩

1. **사용자 초대 시**: 역할과 접근 범위를 명확히 설명
2. **초기 교육**: 시스템 사용법과 보안 수칙 안내
3. **점진적 권한 부여**: 처음에는 제한된 권한으로 시작

#### 조직 변경 대응

1. **부서 이동**: Department 정보 및 프로젝트 접근 권한 업데이트
2. **승진/역할 변경**: Type 및 Role 조정
3. **임시 권한**: 특정 기간 동안만 추가 권한 부여 후 회수
...
| 문제 유형                | 원인 및 해결 방법                        |
| ------------------------ | ---------------------------------------- |
| OAuth 로그인이 작동 안함 | Client ID/Secret 확인, 리디렉션 URI 점검 |
| 사용자 초대 메일 안됨    | SMTP 설정 확인, 스팸함 확인              |
| 권한 변경이 반영 안됨    | 브라우저 새로고침, 재로그인 필요         |

#### 비상 상황 대응

1. **Super Admin 계정 잠금**: 다른 Super Admin 계정으로 복구
2. **OAuth 설정 오류**: 이메일 로그인으로 접근 후 설정 수정
3. **대량 사용자 관리**: CSV 업로드 기능 고려 (향후 기능)

### 지원 요청

문제가 지속되는 경우:

1. **브라우저 개발자 도구**에서 오류 메시지 확인
2. **스크린샷 첨부**하여 정확한 상황 공유
3. **설정 정보 기록**: 민감하지 않은 설정 정보 포함
